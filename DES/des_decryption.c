...
void generate_round_keys(char key[])
{
    ...
    for(int i = 0; i<16; i++)
    {
        ...
        strcat(round_keys[i],"\0");
    }
}

// Function to perform DES decryption
void des_decryption(char cipher_text[],char round_keys[16][49])
{
	...
	for(int i = 0; i < 64; i++)
    {
		strncat(perm, &cipher_text[initial_permutation[i]-1],1); 
	}  
	...
	for(int i = 0; i < 64; i++)
    {
        strncat(decrypted_text, &combined_text[inverse_permutation[i]-1], 1); 
    }
}
...
int main()
{
    ...
    des_decryption(cipher_text,decryption_keys);
